...
<body>
  ...
  if (isset($_SESSION['email'])) {
    ...
    $inner_join = "SELECT products.id,products.name,products.price from user_products INNER JOIN  users on users.id = user_products.user_id
          INNER JOIN products on products.id=user_products.item_id WHERE user_id = $user_id";
      ...
      <div class="col-md-6 offset-1">
        <table class="table table-sm table-responsive-sm">
          ...
          <tbody class="text-center">
            ...
            if (isset($_SESSION['email'])) {
              $user_id = $_SESSION['user_id'];
              $inner_join = "SELECT products.id,products.name,products.price, user_products.status from user_products INNER JOIN  users on users.id = user_products.user_id
                      INNER JOIN products on products.id=user_products.item_id WHERE user_id = $user_id ";
              $inner_join_result = mysqli_query($_con, $inner_join) or die(mysqli_error($_con));
              $row = mysqli_num_rows($inner_join_result);
              if ($row == 0) {
